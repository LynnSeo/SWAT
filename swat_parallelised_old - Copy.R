...
set.seed(1221)
n <- 100
x1 <- parameterSets(getFreeParsRanges(obj),n,method='latin.hypercube')
x2 <- parameterSets(getFreeParsRanges(obj),n,method='latin.hypercube')
# p1=plot(seq(100),x1[1:100,5])
# p2=plot(seq(100),x2[1:100,5])
# # x1=q1
# # x2=q2
incompl = sobol2002(model = NULL,x1, x2, nboot = 1000, conf = 0.95)
# incompl = delsa(model = NULL, perturb=1.01,par.ranges, samples=100,  X0, varprior=1)
...
set.seed(1221)
n <- 100
pars1 <- parameterSets(getFreeParsRanges(obj),n,method='latin.hypercube')

#test response surface
exact_par=read.table('exact parameters.txt')

#read PAWN input
pars1=read.csv('PAWN_pars1.csv')
head(pars1)
...
parss=cbind(pars,exact_par)
head(parss)

colnames(exact_par)

pars=pars1
pars[,2:27]=exact_par[,2:27]
pars[,no.para]=pars1[,no.para]
head(pars)
